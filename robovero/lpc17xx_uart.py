...
class UART1_RS485_CTRLCFG_Type(cstruct):
  ...
  - DirCtrlPol_Level: Polarity of the direction control signal on  the RTS (or DTR) pin:
...
def UART_IrDAPulseDivConfig(UARTx, PulseDiv):
  ...
  - PulseDiv: Pulse Divider value from Peripheral clock, should be one of the following:
...
def UART_FullModemGetStatus(UARTx):
  ...
  - Current value of modem status register
...
def UART_ConfigStructInit(UART_InitStruct):
  ...
  - UART_InitStruct: Pointer to a UART_CFG_Type structure which will be initialized.
